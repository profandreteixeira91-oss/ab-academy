...
function Financeiro({
  ...
  const totalAberto = entries
    .filter((entry) => entry.status === 'pendente')
    .reduce((total, entry) => total + entry.valor, 0)

  const totalVencido = entries
  ...
  const statusLabels: Record<StudentFinanceEntry['status'], string> = {
    pendente: 'Em aberto',
    pago: 'Pago',
    vencido: 'Vencido',
    cancelado: 'Cancelado',
  ...
  if (loading) {
    return (
      <div className="student-finance">
        <div className="student-empty-state">
          <Loader2 size={28} className="student-spin" />
          <h3>Carregando seu financeiro...</h3>
          <p>Aguarde enquanto buscamos seu histórico de pagamentos.</p>
        </div>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="student-finance">
        <div className="student-finance-alert">
          Não foi possível carregar seu histórico financeiro.
          <p>{error}</p>
        </div>
      </div>
    )
    ...
    <div className="student-finance">
      ...
      <div className="student-finance-history">
        <div className="student-finance-history-header">
          <div>
            <span>HISTÓRICO</span>
            <h3>Pagamentos e mensalidades</h3>
          </div>
          <ReceiptText size={22} />
        </div>

        {entries.length === 0 ? (
          <div className="student-finance-empty">
            <ReceiptText size={28} />
            <h3>Nenhum registro financeiro</h3>
            <p>Seu histórico de mensalidades e pagamentos aparecerá aqui.</p>
          </div>
        ) : (
          <div className="student-finance-table-wrap">
            <table className="student-finance-table">
              ...
              <tbody>
                {entries.map((entry) => (
                  <tr key={entry.id}>
                    ...
                    <td>
                      {entry.dataPagamento
                        ? formatDate(entry.dataPagamento)
                        : entry.metodoPagamento || '—'}
                    </td>
                    <td>
                      <span className={`student-finance-status ${entry.status}`}>
