...
function Aluno() {
 ...
 function getLessonAccessMessage(startAt: string, endAt: string) {
  ...
  async function replyStudentRequest() {
    if (!user || !selectedRequest || (!requestMessage.trim() && files.length === 0) || requestSending || selectedRequest.status === 'fechada') return
    try {
      setRequestSending(true)
      setRequestsError('')
      ...
      <div className="student-request-grid">
        <div className="student-request-list">
          <div className="student-request-new">
            ...
            {files.length > 0 && <div className="student-request-file-list">{files.map(file => <span key={file.name + file.size}><Paperclip size={12}/>{file.name}<button type="button" onClick={() => onFilesChange(files.filter(item => item !== file))} aria-label={"Remover " + file.name}><X size={12}/></button></span>)}</div>}
